...
###   schezuancoincore.org and schezuancoin.org and compares them.
...
WORKINGDIR="/tmp/schezuancoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://schezuancoincore.org"
HOST2="https://schezuancoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="schezuancoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'schezuancoin-',
...
if [ $? -ne 0 ]; then
   echo "schezuancoin.org failed to provide signature file, but schezuancoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "schezuancoin.org and schezuancoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Schezuancoin Core binary release signing key installed?"
   fi
