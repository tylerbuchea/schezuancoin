...
The RPM spec file provided here is for Schezuancoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://schezuancoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Schezuancoin git repository](https://github.com/schezuancoin/schezuancoin) but are not part of
...
Currently only `Source10` is used, for the example `schezuancoin.conf` file.
...
in the schezuancoin git repository. At present only an SVG version of the Schezuancoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the schezuancoin
...
needed when 0.12.1 is released, a proper fix is already in the Schezuancoin git
master branch.
...
idea to build Schezuancoin Core against a static version of that library compiled
...
The `schezuancoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `schezuancoin-server` package. At this point, most RPM based
...
The `schezuancoin-server` package should have SELinux support. How to properly do
...
Schezuancoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/schezuancoin/). He has been
packaging Schezuancoin for Fedora at least since 2012.
...
4. I renamed the `schezuancoin` package that contains the Qt GUI to `schezuancoin-core` as
that appears to be how the general population refers to it, in contrast to
`schezuancoin-xt` or `schezuancoin-classic`. I wanted to make sure the general population
