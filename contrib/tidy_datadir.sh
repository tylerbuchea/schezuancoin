#!/bin/bash
# Copyright (c) 2013 The Schezuancoin Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Schezuancoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Schezuancoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Schezuancoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Schezuancoin 0.7 datadir."
    ;;
  3)
    echo "Detected Schezuancoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Schezuancoin 0.8 datadir."
    ;;
esac
